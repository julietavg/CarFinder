...
public class CarService {
    ...
    private static final BigDecimal MAX_PRICE = new BigDecimal("350000.00");
    ...
    public Car createCar(CarDTO carDTO) {
        String vin = carDTO.getVin().toUpperCase();
        if (carRepository.existsByVin(vin)) {
            throw new BusinessRuleViolationException("VIN already exists: " + vin);
        }
        if (carDTO.getPrice().compareTo(MAX_PRICE) > 0) {
            throw new BusinessRuleViolationException("Price exceeds max: " + MAX_PRICE);
        }
    ...
    public Car updateCar(Long id, CarDTO carDTO) {
        ...
        // VIN inmutable (opcional pero recomendable)
        if (!existingCar.getVin().equalsIgnoreCase(carDTO.getVin())) {
            throw new BusinessRuleViolationException("VIN cannot be changed");
        }
        if (carDTO.getPrice().compareTo(MAX_PRICE) > 0) {
            throw new BusinessRuleViolationException("Price exceeds max: " + MAX_PRICE);
        }
    ...
    // ✅ Check if a car exists by VIN
    public boolean existsByVin(String vin) {
        return carRepository.existsByVin(vin.toUpperCase());
    }
}
